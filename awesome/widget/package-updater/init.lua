...
local return_button = function()
	...
	watch(
		"sudo apt update | tail -n1 | egrep -o '^[0-9]*'",
		3600, -- check updates per hour
		function(_, stdout)
			number_of_updates_available = stdout
			local icon_name = nil
			if number_of_updates_available ~= nil then
				update_available = true
				icon_name = 'package-up'
                local handle = io.popen('apt list --upgradable')
                if handle ~= nil then
                    update_package = handle:read('*a')
                    handle:close()
                end
			else
